...
backbone.sync = function(method, model, success, error) {
  when(when(http.request(model.url), success, error), null, function(error) {
    console.error(error)
  })
}
...
, parse: function parse(content) {
    ...
    for (var name in packages) {
      ...
      // Mapping modules to template friendly format.
      data.modulesTemplate = _.map(_.keys(data.modules), function(id) {
        id = id ? name + '/' + id : name
        return {
          id: id,
      ...
      // Mapping dependencies to template friendly format.
      data.dependenciesTemplate = _.map(_.keys(data.dependencies), function(name) {
        return {
          name: name,
...
, template:
  ...
  , '<br/>'
  , '<div class="keywords">{{#keywords}}<code class="tag">{{.}}</code>\n{{/keywords}}</div><br/>'
  ...
  , '{{dependenciesTemplate.length}}<div class="dependencies">Dependencies: {{#dependenciesTemplate}}'
    ,   '<a href="#packages/{{name}}" class="module fixed">{{name}} {{version}}</a>'
  , '{{/dependenciesTemplate}}</div>'
  , '<div class="modules">Modules: {{#modulesTemplate}}'
    ,   '<a href="#packages/{{name}}/modules/{{src}}" class="module fixed">{{id}}</a>\n'
  , '{{/modulesTemplate}}</div>'
  ].join('\n')
, render: function render() {
